...
def assemble_A_local(cross_R_t1, cross_R_t2, cross_R_n, point_t1, point_t2, point_n):
    Alocal = np.matrix([
        [-1*point_t1[0], -1*point_t2[0], -1*point_n[0]],
        [-1*point_t1[1], -1*point_t2[1], -1*point_n[1]],
        [-1*point_t1[2], -1*point_t2[2], -1*point_n[2]],
...
def cal_A_global_3d(elems, contps, sparse=False):
    if not sparse:
        ...
        cross_R_t1, cross_R_t2, cross_R_n = batch_cross(R_all_points, t1_all_points, t2_all_points, n_all_points)
        ...
        for p_c in tqdm(contps.values(),total=len(contps),desc='Calculating A matrix'):
            ...
            cand_element = elems[p_c.cand]
            anta_element = elems[p_c.anta]
            Alocal1 =assemble_A_local(cross_R_t1[i,:], cross_R_t2[i, :], cross_R_n[i, :], t1_all_points[i, :], t2_all_points[i, :], n_all_points[i, :])
            i += 1
            Alocal2 =assemble_A_local(cross_R_t1[i,:], cross_R_t2[i, :], cross_R_n[i, :], t1_all_points[i, :], t2_all_points[i, :], n_all_points[i, :])
            i += 1
